...
frappe.query_reports["EITS Payment Image"] = {
    ...
    "formatter": function(value, row, column, data, default_formatter) {
        ...
        if (column.fieldname === "view_images" && value === "View Images") {
            return `<button class="btn btn-sm" onclick="view_payment_images('${data.name}'); return false;" style="cursor: pointer;">
        ...
        if (column.fieldname === "submit_action") {
            if (value === "Submit") {
                return `<button class="btn btn-sm" onclick="submit_payment('${data.name}'); return false;" style="cursor: pointer;">
...
// Function to submit payment - THIS WAS MISSING IN YOUR ORIGINAL CODE
function submit_payment(payment_id) {
    // Show confirmation dialog
    frappe.confirm(
        __('Are you sure you want to submit payment {0}?', [payment_id]),
        function() {
            // Show loading message
            frappe.show_alert({
                message: __('Submitting payment...'),
                indicator: 'blue'
            });

            // Call the server method
            frappe.call({
                method: "eits_app.eits_app.report.eits_payment_image.eits_payment_image.submit_payment",
                args: {
                ...
                callback: function(r) {
                    if (r.message && r.message.success) {
                        ...
                        frappe.msgprint({
                            title: __("Error"),
                            message: r.message ? r.message.message : __("Failed to submit payment. Please try again."),
                            indicator: "red"
                        });
                    }
                },
                error: function(r) {
                    console.error("Error submitting payment:", r);
                    let error_message = __("Error submitting payment: ");
...
// Function to fetch and display payment images using server method
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (r.message && r.message.length > 0) {
                ...
                frappe.msgprint({
                    title: __("No Images"),
                    message: __("No images found for this payment record."),
                    indicator: "orange"
                });
            }
        },
...
function show_image_dialog(payment_id, images) {
    ...
    if (validImages.length === 0) {
        frappe.msgprint({
            title: __("No Images"),
            message: __("No valid images found for this payment record."),
            indicator: "orange"
        });
        return;
    }
    ...
    create_image_carousel_panel(payment_id, validImages);
}

// Function to create the side panel image carousel
function create_image_carousel_panel(payment_id, images) {
    ...
    let panelHtml = `
        ...
        ">
            ...
            <div class="carousel-header">
                <div>
                    <h4 style="margin: 0; font-size: 18px;">Payment Images: ${payment_id}</h4>
                </div>
            ...
            <div class="carousel-content">
                ...
                <img id="carousel-image" class="carousel-image" src="${images[0].image}" alt="Payment Image">
...
function test_submit_method(payment_id) {
    console.log("Testing submit method for:", payment_id);
    frappe.call({
        method: "eits_app.eits_app.report.eits_payment_image.eits_payment_image.submit_payment",
        args: {
            payment_id: payment_id
